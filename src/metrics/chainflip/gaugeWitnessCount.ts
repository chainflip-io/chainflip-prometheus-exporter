...
export const gaugeWitnessCount = async (context: Context, data: ProtocolData): Promise<void> => {
    ...
    if (global.epochIndex) {
        ...
        try {
            ...
            processHash10(currentBlockNumber, apiLatest, logger, data.blockHash, data.apiAt);
            processHash50(currentBlockNumber, apiLatest, logger, data.blockHash, data.apiAt);
