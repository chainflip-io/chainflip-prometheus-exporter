...
let witnessHashEth: Set<any> = new Set;
let witnessHashBtc: Set<any> = new Set;
let witnessHashDot: Set<any> = new Set;
    ...
    if(global.epochIndex){
        ...
        try {
            ...
            for (const elem of witnessHashEth) {
                const votes: string = (await api.query.witnesser.votes(global.epochIndex, elem)).toHuman();
                let binary = hex2bin(votes);
                let number = binary.match(/1/g)?.length;
                witnessNumber.push(number ? number : 0);
            }
            ...
            signedBlock.block.extrinsics.forEach((ex: any, index: any) => {
                ...
                if(ex.toHuman().method.method == 'witnessAtEpoch') {
                    ...
                    if(callData && callData.section == 'polkadotChainTracking'){
                        ...
                        // parse the data and removed useless comas (damn polkadot api)
                        const blockHeight = finalData.new_chain_state.blockHeight.replace(/,/g, '');
                        const runtimeVersion = finalData.new_chain_state.trackedData.runtimeVersion.specVersion.replace(/,/g, '');
